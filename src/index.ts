...
async function main() {
  ...
  debugLog('Environment:', CONFIG.SERVER.NODE_ENV, CONFIG.IS_DEV ? '(debug mode)' : '');
  ...
  bot.catch(async (err) => {
    ...
    if (err.ctx && err.ctx.chat) {
      try {
        await err.ctx.reply('⚠️ Sorry, something went wrong. Please try again.');
      } catch (replyError) {
  ...
  try {
    ...
    if (usersWithTokens.length > 0) {
      console.log(`📊 Restored ${usersWithTokens.length} authenticated users from database`);
      debugLog('Users with tokens:', usersWithTokens.map(u => u.userId));
    } else {
